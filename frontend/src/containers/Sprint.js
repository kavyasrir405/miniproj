...
const Sprint = ({ sprint, fetchData, onSprintDelete, setissueDragged = null, onissueTypeChange,toggleTrigger }) => {
  ...
  const fetchIssues = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}:8000/djapp/issuesOfSprint/`, {
  ...
  const handleSprintForm = async () => {
    if (buttonType === 'start') {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}:8000/djapp/updateSprintStatus/`, {
  ...
  const handleDeleteSprint = async () => {
    ...
    if (confirmDelete) {
      try {
        await axios.get(`${process.env.REACT_APP_API_URL}:8000/djapp/delete_sprint/`, {
          params: { projectId: projectid, sprintName: sprint.sprint }
        });
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: async (item) => {
      ...
      await axios.post(`${process.env.REACT_APP_API_URL}:8000/djapp/update_issueSprint/`, { issue: item.issue.IssueName, sprint: sprint.sprint, projectId: projectid });
