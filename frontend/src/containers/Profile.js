...
const Profile = ({ user }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`http://${process.env.REACT_APP_API_URL}:8000/djapp/get_user_profile/`, {
  ...
  const handleSubmit = async (e, field) => {
    ...
    try {
      const response = await axios.put(
        `http://${process.env.REACT_APP_API_URL}:8000/djapp/update_user_profile/`,
